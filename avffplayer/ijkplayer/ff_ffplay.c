        ...
        switch (d->avctx->codec_type) {
            case AVMEDIA_TYPE_VIDEO: {
                ...
                if (got_frame) {
                    ...
                    if (fn_on_in_frame)
                    {
                        fn_on_in_frame(FF_PKT_TYPE_VIDEO, frame, d->avctx, video_fps);
                    }

                    //post_decoded_avframe(FF_PKT_TYPE_VIDEO, frame);
                }
                }
                break;
            case AVMEDIA_TYPE_AUDIO:
                ...
                if (got_frame) {
                    ...
                    if (fn_on_in_frame)
                    {
                        fn_on_in_frame(FF_PKT_TYPE_AUDIO, frame, d->avctx, video_fps);
                    }
                    //post_decoded_avframe(FF_PKT_TYPE_AUDIO, frame);
                }
                break;
...
static int stream_component_open(FFPlayer *ffp, int stream_index)
{
    ...
    case AVMEDIA_TYPE_AUDIO:
        ...
        if (fn_on_in_codec_parsed)
        {
            fn_on_in_codec_parsed(FF_PKT_TYPE_AUDIO, avctx, 0);
        }

        //on_in_codec_ffplay_parsed(FF_PKT_TYPE_AUDIO, avctx, 0);
    ...
    case AVMEDIA_TYPE_VIDEO:
        ...
        if (fn_on_in_codec_parsed)
        {
            fn_on_in_codec_parsed(FF_PKT_TYPE_VIDEO, avctx, video_fps);
        }
        //on_in_codec_ffplay_parsed(FF_PKT_TYPE_VIDEO, avctx, video_fps);
...
void on_pkt_queue(int type, AVPacket *pkt)
{
//    post_av_packet(type, pkt);
}
...
static int read_thread(void *arg)
{
    ...
    av_log(NULL, AV_LOG_FATAL, "ST NATIVE: %s\n", "read_thread started");
    ...
    if (st_index[AVMEDIA_TYPE_AUDIO] >= 0) {
        ...
        av_log(NULL, AV_LOG_FATAL, "ST NATIVE: %s\n", "stream_component_open AVMEDIA_TYPE_AUDIO");
    }

    ret = -1;
    if (st_index[AVMEDIA_TYPE_VIDEO] >= 0) {
        ret = stream_component_open(ffp, st_index[AVMEDIA_TYPE_VIDEO]);
        av_log(NULL, AV_LOG_FATAL, "ST NATIVE: %s\n", "stream_component_open AVMEDIA_TYPE_VIDEO");
    }
    ...
    if (is->video_st && is->video_st->codecpar) {
        ...
        av_log(NULL, AV_LOG_FATAL, "ST NATIVE: VIDEO_SIZE %d  x  %d\n", codecpar->width, codecpar->height);
...
//      of the seek_pos/seek_rel variables
            ...
            if (ret < 0) {
                ...
                if (is->audio_stream >= 0) {
                    ...
                    on_pkt_queue(FF_PKT_TYPE_AUDIO, &flush_pkt);
                }
#ifdef FFP_MERGE
                if (is->subtitle_stream >= 0) {
                ...
                if (is->video_stream >= 0) {
                    ...
                    on_pkt_queue(FF_PKT_TYPE_VIDEO, &flush_pkt);
                }
        ...
        if (is->queue_attachments_req) {
            if (is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC)) {
                ...
                on_pkt_queue(FF_PKT_TYPE_VIDEO, &copy);

                packet_queue_put_nullpacket(&is->videoq, is->video_stream);
                on_pkt_queue(FF_PKT_TYPE_VIDEO, NULL);
            }
        ...
        if (ret < 0) {
            ...
            if (pb_eof) {
                ...
                if (is->audio_stream >= 0)
                {
                    ...
                    on_pkt_queue(PKT_TYPE_AUDIO, NULL);
                }
#ifdef FFP_MERGE
                if (is->subtitle_stream >= 0)
                {
        ...
        if (pkt->flags & AV_PKT_FLAG_DISCONTINUITY) {
            if (is->audio_stream >= 0) {
                packet_queue_put(&is->audioq, &flush_pkt);
                on_pkt_queue(PKT_TYPE_AUDIO, &flush_pkt);
            }
#ifdef FFP_MERGE
            if (is->subtitle_stream >= 0) {
                packet_queue_put(&is->subtitleq, &flush_pkt);
        ...
        if (pkt->stream_index == is->audio_stream && pkt_in_play_range) {
            ...
            on_pkt_queue(FF_PKT_TYPE_AUDIO, pkt);
        } else if (pkt->stream_index == is->video_stream && pkt_in_play_range
                   && !(is->video_st && (is->video_st->disposition & AV_DISPOSITION_ATTACHED_PIC))) {
            packet_queue_put(&is->videoq, pkt);
            on_pkt_queue(FF_PKT_TYPE_VIDEO, pkt);
#ifdef FFP_MERGE
...
void set_in_codec_parse_callback(void (*f)(int, AVCodecContext*, int) )
{
    fn_on_in_codec_parsed = f;
}

void set_in_frame_callback(void (*f)(int, AVFrame*, AVCodecContext*, int) )
{
    fn_on_in_frame = f;
}
